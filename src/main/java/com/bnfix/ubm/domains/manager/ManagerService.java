...
public class ManagerService {
    ...
    @Transactional
    public ManagerResponse create(CreateManagerRequest r) {
        ...
        Account a = accounts.save(
                Account.builder(r.name(), CPF.of(r.cpf()), passwords.encode(r.password()), r.email(), Role.MANAGER)
                        .build());
        return response(managers.save(Manager.builder(r.name(), c, a).build()));
    }

    @Transactional
    public ManagerResponse me(String email, Long id) {
        return response(active(email, id));
    }

    @Transactional
    public ManagerResponse updateEmail(String email, Long id, UpdateManagerEmailRequest r) {
        Manager m = active(email, id);
        String normalized = r.email().trim().toLowerCase(Locale.ROOT);
        checkPassword(m, r.currentPassword());
        if (!m.getAccount().getEmail().equalsIgnoreCase(normalized)
                && accounts.findByEmail(normalized).isPresent()) throw conflict("Email already in use");
        m.getAccount().updateEmail(normalized);
        return response(m);
    }

    @Transactional
    public ManagerResponse changePassword(String email, Long id, ChangeManagerPasswordRequest r) {
        Manager m = active(email, id);
        checkPassword(m, r.currentPassword());
        if (passwords.matches(r.newPassword(), m.getAccount().getPassword()))
            throw bad("New password must be different");
        m.getAccount().updatePassword(passwords.encode(r.newPassword()));
        return response(m);
    }

    private Manager active(String email, Long id) {
        return AccessStatusGuard.requireActive(
                managers.findByEmailAndCompanyId(email, id).orElseThrow(() -> notFound("Manager not found")));
    }

    private void checkPassword(Manager m, String p) {
        if (!passwords.matches(p, m.getAccount().getPassword())) throw bad("Password is incorrect");
    }

    private ManagerResponse response(Manager m) {
        return new ManagerResponse(
                m.id, m.getName(), m.getAccount().getEmail(), m.getCompany().id, m.getActive(), m.getCreatedAt());
    }
