...
public class AnnouncementController {
    ...
    public ResponseEntity<?> publish(
        ...
        return ResponseEntity.status(201).body(service.publish(auth.getName(), companyId(jwt), request));
    }
    ...
    public ResponseEntity<?> listCompany(
            ...
            Authentication auth,
            @AuthenticationPrincipal Jwt jwt) {
        return ResponseEntity.ok(service.listCompany(auth.getName(), companyId(jwt), page, size));
    }
    ...
    public ResponseEntity<?> listMine(
            ...
            Authentication auth,
            @AuthenticationPrincipal Jwt jwt) {
        return ResponseEntity.ok(service.listMine(auth.getName(), companyId(jwt), page, size));
    }

    @GetMapping("/me/unread-count")
    @PreAuthorize("hasRole('USER')")
    public ResponseEntity<?> unreadCount(Authentication auth, @AuthenticationPrincipal Jwt jwt) {
        return ResponseEntity.ok(service.unreadCount(auth.getName(), companyId(jwt)));
    }
    ...
    public ResponseEntity<?> markRead(
        ...
        return ResponseEntity.ok(service.markRead(auth.getName(), companyId(jwt), announcementId));
    }

    @PutMapping("/me/read-all")
    @PreAuthorize("hasRole('USER')")
    public ResponseEntity<?> markAllAsRead(Authentication auth, @AuthenticationPrincipal Jwt jwt) {
        return ResponseEntity.ok(service.markAllAsRead(auth.getName(), companyId(jwt)));
    }
